...
function AbsentDatesCell({ dates }) {
  ...
  return (
    <>
      <Button size="small" startIcon={<CalendarMonthIcon fontSize="small" />}
        onClick={(e) => setAnchor(e.currentTarget)} sx={{ textTransform: 'none' }}>
        View ({list.length})
      </Button>
      <Menu anchorEl={anchor} open={!!anchor} onClose={() => setAnchor(null)}
        PaperProps={{ sx: { maxHeight: 320 } }}>
        {list.map((d) => (
          <MenuItem key={d} dense disableRipple sx={{ fontSize: 13 }}>{d}</MenuItem>
        ))}
      </Menu>
    </>
  );
  ...
  return (
    ...
    >
      ...
      <LinearProgress variant="determinate" value={Math.min(rate, 100)}
        sx={{
          height: 6, borderRadius: 3, bgcolor: 'grey.100', mt: 1,
      ...
      <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 0.5 }}>
        {outlet.total_emp} staff · {outlet.absent_days} absent days
      </Typography>
    </Box>
  );
}

export default function AdminOutletsDetail() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get('/report/outlet-summary/outlets/', { params });
      setOutlets(res.data || []);
    } catch (err) {
  ...
  const donutData = useMemo(() => {
    ...
    return [
      ...
      { name: 'Absent (blank)', value: Number(selectedOutlet.absent_days || 0), color: '#DC2626' },
    ].filter((d) => d.value > 0);
  }, [selectedOutlet]);
  ...
  return (
    <Box sx={{ width: '95%', mx: 'auto', mt: 4, display: 'flex', flexDirection: 'column', gap: 3 }}>
      <PageHeader
        title="Outlets Detail"
        subtitle="Per-outlet attendance breakdown. Absent = blank dates (no attendance and no leave) inside the selected range."
        actions={
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: selectedOutlet ? '1fr 1fr' : '1fr' }, gap: 2.5 }}>
        <Box sx={{ bgcolor: 'background.paper', border: 1, borderColor: 'divider', borderRadius: 2.5, p: 2.5 }}>
            ...
            <Box sx={{ ml: 'auto' }}>
              <FormControl size="small" sx={{ minWidth: 170 }}>
                ...
                <Select label="Sort by" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                  ...
                  <MenuItem value="absent_desc">Most blank dates</MenuItem>
          ...
          <Box sx={{
            ...
            {sortedOutlets.map((o) => (
              <OutletCard key={o.outlet_id} outlet={o}
